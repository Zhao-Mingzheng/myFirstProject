...
def load_model_scaler():
    model = joblib.load(r"D:\ST\web\web\static\best_model_XGBoost.joblib")
...
def load_background_data():
    df = pd.read_excel(r"D:\ST\web\web\static\L34v9re13.xlsx", sheet_name='Sheet1').dropna()
...
def main():
    ...
    if st.sidebar.button('Submit'):
        ...
        print(proba)
        label = np.array([(proba >= 0.19).astype(int)])
